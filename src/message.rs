                    ...
                    && item_message[item_index].item_size == private_message)
            {
                ...
                match results {
                    Ok((_, formatted_message)) => {
                        formatted_log_message = rc_string!(formatted_message)
                    }
...
fn parse_formatter<'a>(
    ...
    if PRECISION_ITEMS.contains(&item_type) {
        ...
        if index >= message_value.len() {
            ...
            return Ok(("", rc_string!("Failed to format string due index length")));
        }
    }
        ...
        && number_item_type.contains(&message_value[index].item_type)
    {
        ...
        match char_results {
            ...
            Err(err) => {
                ...
                return Ok(("", rc_string!("Failed to parse number item to char string")));
            }
        }
    }
    ...
    if ERROR_TYPES.contains(&type_data) {
        message = rc_string!(format!("Error code: {message}"));
        return Ok(("", message));
    }
...
fn parse_type_formatter<'a>(
    ...
    if format_type.contains("signpost") {
        ...
        message = rc_string!(format!("{message} ({signpost_message})"));
    }
    Ok(("", message))
}
...
fn parse_float(message: &str) -> f64 {
...
mod tests {
    ...
    fn test_format_firehose_log_message() {
        ...
        let log_string =
            format_firehose_log_message(rc_string!(test_data), &item_message, &message_re);
        assert_eq!(
            log_string.as_str(),
    ...
    fn test_bad_format_options() {
        ...
        let log_string = format_firehose_log_message(rc_string!(message), &items, &message_re);
    ...
    fn test_parse_formatter() {
        ...
        .unwrap();
        assert_eq!(formatted_results.as_str(), "+002");
        ...
        .unwrap();
        assert_eq!(formatted_results.as_str(), "0o144");
        ...
        .unwrap();
        assert_eq!(formatted_results.as_str(), "+000.0035");
        ...
        .unwrap();
        assert_eq!(
            formatted_results.as_str(),
            "The big red dog jumped over the crab"
        );
        ...
        .unwrap();
        assert_eq!(formatted_results.as_str(), "aa");
        ...
        .unwrap();
        assert_eq!(formatted_results.as_str(), "        hi");
    }
    ...
    fn test_parse_type_formatter() {
        ...
        .unwrap();
        assert_eq!(formatted_results.as_str(), "test");
    ...
    #[test_case("2", FormatableType::Integer, 0, 4, false, false, "2000"; "WAT ?")]
    #[test_case("2", FormatableType::Integer, 0, 5, false, false, "20000";"WATT ?")]
    #[test_case("2", FormatableType::Integer, 0, 3, true, false, "+20"; "WATTT ?")]
    #[test_case("2", FormatableType::Integer, 0, 0, true, false, "+2")]
    #[test_case("plop", FormatableType::Integer, 0, 3, false, false, "000"; "error case => defaults to 0")]
    #[test_case(F, FormatableType::Float, 4, 10, true, false, "+2.1000000")]
    fn test_format_alignment_left(
        ...
        let result =
            format_alignment_left(message, width, precision, formatable, plus_minus, hashtag);
        assert_eq!(result.as_str(), expected);
    }

    #[test_case("2", FormatableType::Integer, 0, 4, false, false, "0002")]
    #[test_case("2", FormatableType::Integer, 0, 5, false, false, "00002")]
    #[test_case("2", FormatableType::Integer, 0, 3, true, false, "+02")]
    #[test_case("2", FormatableType::Integer, 0, 0, true, false, "+2")]
    #[test_case("plop", FormatableType::Integer, 0, 3, false, false, "000"; "error case => defaults to 0")]
    #[test_case(F, FormatableType::Float, 4, 10, true, false, "+0002.1000")]
    fn test_format_alignment_right(
        ...
        let message = rc_string!(message);
        let result =
            format_alignment_right(message, width, precision, formatable, plus_minus, hashtag);
        assert_eq!(result.as_str(), expected);
    }
