...
mod tests {
    ...
    fn test_fill_private_data_mixed() {
        ...
        // with size 0, PrivateNumber 0x01 with size 0x8000).
        let test_data: &[u8] = &[
        ...
        let mut items = [
            // 0x45 Sensitive — should stay Private
            RawFirehoseItem {
        ...
        for (i, it) in items.iter().enumerate().take(4) {
            assert!(
                matches!(it.value, RawItemValue::Private { .. }),
                "item {i} should stay Private, got {:?}",
                it.value
            );
        }

        // Item 4 (0x81) should be filled with the RTErrorDomain string
        let expected = "Error Domain=RTErrorDomain Code=2 \"Service has been disabled by user.\" UserInfo={NSLocalizedDescription=Service has been disabled by user.}";
        match &items[4].value {
