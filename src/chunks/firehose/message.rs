...
impl MessageData {
    ...
    pub fn extract_shared_strings<'a>(
        ...
        if let Some(dsc_uuid) = dsc_uuid
            && provider.cached_dsc(dsc_uuid).is_none()
        {
            ...
            for ranges in &shared_string.ranges {
                    ...
                    && string_offset < (ranges.range_offset + u64::from(ranges.range_size))
                {
                    ...
                    */
                    ...
                    message_data.format_string = rc_string!(message_string);
            ...
            && let Some(shared_string) = provider.cached_dsc(dsc_uuid)
        {
            ...
            if let Some(ranges) = shared_string.ranges.first() {
                ...
                shared_string.uuids[ranges.unknown_uuid_index as usize]
                    .uuid
                    .clone_into(&mut message_data.library_uuid);
                message_data.format_string = rc_string!("Error: Invalid shared string offset");
                message_data.process_uuid = main_uuid;
        ...
        message_data.format_string = rc_string!("Unknown shared string message");
    ...
    pub fn extract_format_strings<'a>(
        ...
        if let Some(data) = provider.cached_uuidtext(message_data.process_uuid) {
            ...
            message_data.process = rc_string!(process_string);
            message_data.library = rc_string!(process_string);
            message_data.format_string = rc_string!(format!(
    ...
    pub fn extract_absolute_strings<'a>(
            ...
            && let Some(data) = provider.cached_uuidtext(message_data.library_uuid)
        {
            ...
            let (_, library_string) =
                MessageData::uuidtext_image_path(footer_data, &data.entry_descriptors)?;
            message_data.library = rc_string!(library_string);
    ...
    pub fn extract_alt_uuid_strings<'a>(
        ...
        if let Some(data) = provider.cached_uuidtext(uuid) {
            ...
            message_data.library = rc_string!(library_string);
            message_data.format_string = rc_string!(format!(
                "Error: Invalid offset {string_offset} for alternative UUID {uuid}"
            ));
        ...
        message_data.format_string = rc_string!(format!(
            "Failed to get string message from alternative UUIDText file: {uuid}"
        ));
        Ok((&[], message_data))
    }
    ...
    fn get_catalog_dsc(
        ...
        let mut dsc_uuid = None;
        let mut main_uuid = Uuid::nil();

        if let Some(entry) = catalogs
            .catalog_process_info_entries
            .get(&CatalogProcessInfoKey(first_proc_id, second_proc_id))
        {
            main_uuid = entry.main_uuid;
            dsc_uuid = entry.dsc_uuid;
        }
        (dsc_uuid, main_uuid)
    }
}
...
mod tests {
    ...
    fn test_extract_absolute_strings_nonactivity_bad_offset() {
        ...
        .unwrap();
        assert_eq!(results.library.as_str(), "");
        assert_eq!(
            results.format_string.as_str(),
    ...
    fn test_extract_alt_uuid_strings() {
        ...
        let test_uuid = Uuid::parse_str("C275D5EEBAD43A86B74F16F3E62BF57D").unwrap();
        ...
        assert_eq!(
            results.library_uuid,
            Uuid::parse_str("C275D5EEBAD43A86B74F16F3E62BF57D").unwrap()
        );

        assert_eq!(results.process.as_str(), "/usr/libexec/opendirectoryd");
        assert_eq!(
            results.process_uuid,
    ...
    fn test_get_catalog_dsc() {
        ...
        assert_eq!(
            dsc_uuid,
            Some(Uuid::parse_str("80896B329EB13A10A7C5449B15305DE2").unwrap())
        );
        assert_eq!(
            main_uuid,
            Uuid::parse_str("87721013944F3EA7A42C604B141CCDAA").unwrap()
        );
    }
    ...
    fn test_get_uuid_image_path() {
        ...
        let test_uuid = Uuid::parse_str("B736DF1625F538248E9527A8CEC4991E").unwrap();
