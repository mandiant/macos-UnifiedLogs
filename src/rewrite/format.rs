...
fn apply_hex_format(output: &mut String, n: i64, spec: &FormatSpec) {
  ...
  let hex_str = if spec.alternate { format!("0x{:X}", n) } else { format!("{:X}", n) };
...
fn apply_octal_format(output: &mut String, n: i64, spec: &FormatSpec) {
  ...
  let oct_str = if spec.alternate { format!("{n:#o}") } else { format!("{n:o}") };
...
fn parse_specifier(bytes: &[u8]) -> (FormatSpec, usize, bool) {
  ...
  if pos < len && bytes[pos] == b'*' {
    ...
    if pos > start
      && let Ok(w) = std::str::from_utf8(&bytes[start..pos]).unwrap_or("0").parse::<usize>()
    {
      spec.width = w;
      spec.has_width = true;
    }
  ...
  if pos < len && is_length_modifier_start(bytes[pos] as char) {
    ...
    if pos < len && ((bytes[pos - 1] == b'h' && bytes[pos] == b'h') || (bytes[pos - 1] == b'l' && bytes[pos] == b'l')) {
      pos += 1;
    }
  }
...
pub fn format_message(format_string: Option<&str>, items: &[RawFirehoseItem<'_>], decoder: &dyn AppleDecoder) -> String {
  ...
  while pos < len {
    ...
    if bytes[pos] == b'{' {
      ...
      if pos >= len || !is_conversion_char(bytes[pos] as char) {
        ...
        format_annotated_item(&mut result, &annotation, &spec, items, &mut item_index, decoder);
        continue;
      }
