...
pub fn visit_logarchive(path: &Path, mut callback: impl for<'a, 'b> FnMut(LogEntry<'a, 'b>)) -> Result<(), std::io::Error> {
  ...
  for tracev3_path in &tracev3_paths {
    ...
    if let Err(e) = visit_tracev3(&data, &resolver, &dsc_files, &uuidtext_files, &mut oversize_cache, |entry| {
      callback(entry);
    }) {
...
mod tests {
  ...
  fn test_collect_tracev3_paths_order() {
    ...
    assert!(first.contains("Persist"), "first tracev3 should be from Persist/, got: {first}");

    // LiveData should be last
    let last = paths.last().unwrap().to_string_lossy();
    assert!(last.contains("LiveData"), "last tracev3 should be LiveData, got: {last}");
  }
}
