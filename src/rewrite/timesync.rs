...
impl TimestampResolver {
  ...
  pub fn resolve(&self, boot_uuid: &Uuid, absolute_continuous_time: u64, preamble_time: u64) -> f64 {
    ...
    let continuous_time =
      (absolute_continuous_time as f64).mul_add(timebase_adjustment, -(timesync_continuous_time as f64) * timebase_adjustment);
...
mod tests {
  ...
  fn test_parse_timesync_record() -> anyhow::Result<()> {
    let test_data: [u8; 32] = [
      84, 115, 32, 0, 0, 0, 0, 0, 165, 196, 104, 252, 1, 0, 0, 0, 216, 189, 100, 108, 116, 158, 131, 22, 0, 0, 0, 0, 0, 0, 0, 0,
    ];
  ...
  fn test_parse_timesync_boot() -> anyhow::Result<()> {
    let test_data: [u8; 48] = [
      176, 187, 48, 0, 0, 0, 0, 0, 132, 91, 13, 213, 1, 96, 69, 62, 172, 224, 56, 118, 12, 123, 92, 29, 1, 0, 0, 0, 1, 0, 0, 0, 168, 167,
      19, 176, 114, 158, 131, 22, 0, 0, 0, 0, 0, 0, 0, 0,
    ];
    let (remaining, boot) = RawTimesyncBoot::parse(&test_data).unwrap();
    assert!(remaining.is_empty());
    assert_eq!(boot.boot_uuid, Uuid::parse_str("845B0DD50160453EACE038760C7B5C1D")?);
  ...
  fn test_parse_timesync_file() -> anyhow::Result<()> {
    let test_path = test_data_path().join("system_logs_big_sur.logarchive/timesync/0000000000000002.timesync");
