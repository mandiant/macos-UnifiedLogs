...
pub enum ParseError {
  #[error("Unexpected EOF, tried to read at offset {offset:#x}: need {needed:?} bytes, only {available:?} in {context:?}")]
  UnexpectedEof {
    offset: Offset,
    needed: Option<usize>,
...
impl ParseError {
  pub fn unexpected_eof(offset: Offset, needed: usize, available: usize, context: Option<&'static str>) -> Self {
    Self::UnexpectedEof {
      offset,
      needed: Some(needed),
  ...
  pub fn unknown_chunk_tag(offset: Offset, tag: u32, context: Option<&'static str>) -> Self {
    Self::UnknownChunkTag { offset, tag, context }
  }
