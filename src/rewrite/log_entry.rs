...
impl<'a, 'b> LogEntry<'a, 'b> {
    ...
    pub fn message(&self) -> Rc<String> {
        if let Some(msg) = self.resolved_message.borrow().as_ref() {
            return msg.clone();
        }
        ...
        let msg = Rc::new(msg);
        *self.resolved_message.borrow_mut() = Some(msg.clone());
        msg
    }
...
fn format_statedump_object(data: &[u8], title_name: &str) -> String {
    let result = match title_name {
        "CLDaemonStatusStateTracker" => {
            location::get_daemon_status_tracker(data).map(|(_, r)| r.to_string())
        }
        "CLClientManagerStateTracker" => {
            location::get_state_tracker_data(data).map(|(_, r)| r.to_string())
        }
