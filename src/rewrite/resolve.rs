...
fn resolve_absolute<'a>(
  ...
  let library_uuid = entry
    .and_then(|e| {
      e.uuid_info_entries
        .iter()
        .find(|u| absolute_offset >= u.load_address && absolute_offset <= (u.load_address + u64::from(u.size)))
        .map(|u| u.uuid)
    })
    .unwrap_or(Uuid::nil());
...
mod tests {
  ...
  fn test_resolve_missing_uuid() {
    ...
    let result = resolve_strings(12345, 0, &formatter, 99, 99, &catalog, &dsc_files, &uuidtext_files);
