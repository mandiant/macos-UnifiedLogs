...
use crate::rewrite::helpers::{decode_standard, extract_string, extract_string_size};
...
fn remove_err_offset(error_with_offset: nom::Err<nom::error::Error<(&[u8], usize)>>) -> nom::Err<nom::error::Error<&[u8]>> {
  use nom::Err;
...
pub(crate) fn dns_idflags(input: &str) -> Result<DnsIdFlags, DecoderError<'_>> {
  ...
  bytes.as_mut().write_u32::<BigEndian>(flags).map_err(|_| DecoderError::Parse {
    input: input.as_bytes(),
    parser_name: "dns id flags",
...
impl Display for DnsIdFlags {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    if let Some(decoded_flags) = &self.decoded_flags {
      write!(f, "id: {:#X?}, flags: {:#X?} {decoded_flags}", self.id, self.flags)
    } else {
      write!(f, "id: {:#X?}, flags: {:#X?} Failed to parse ID Flags", self.id, self.flags)
    }
  }
}
...
pub(crate) fn dns_counts(input: &str) -> Result<DnsCounts, DecoderError<'_>> {
  ...
  bytes.as_mut().write_u64::<BigEndian>(flags).map_err(|_| DecoderError::Parse {
    input: input.as_bytes(),
    parser_name: "dns counts",
...
mod tests {
  ...
  fn test_parse_svcb_ip() {
    let test_data = [
      0, 4, 0, 8, 104, 16, 148, 64, 104, 16, 149, 64, 0, 6, 0, 32, 38, 6, 71, 0, 0, 0, 0, 0, 0, 0, 0, 0, 104, 16, 148, 64, 38, 6, 71, 0, 0,
      0, 0, 0, 0, 0, 0, 0, 104, 16, 149, 64,
    ];
