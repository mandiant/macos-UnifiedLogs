...
pub(crate) fn location_manager_state_tracker_state(input: &str) -> Result<LocationTrackerState, DecoderError<'_>> {
  ...
  let (_, result) = get_location_tracker_state(&decoded_data).map_err(|_| DecoderError::Parse {
    input: input.as_bytes(),
    parser_name: "location manager state tracker state",
...
pub(crate) fn get_location_tracker_state(input: &[u8]) -> nom::IResult<&[u8], LocationTrackerState> {
  ...
  let (
    input,
    (distance_filter, desired_accuracy, updating_location, requesting_location, requesting_ranging, updating_ranging, updating_heading),
  ) = accuracy_tup.parse(input)?;

  let mut dynamic_tup = (take(UNKNOWN_DATA_LENGTH), le_f64, le_u8, le_u8, le_u8);
  let (input, (_unknown, heading_filter, allows_location_prompts, allows_altered_locations, dynamic_accuracy)) =
    dynamic_tup.parse(input)?;

  let mut track_tup = (le_u8, le_i32, le_u8, take(UNKONWN_DATA_LENGTH2), le_i64, le_i32);
  let (
    input,
    (
  ...
  let (input, (paused, allows_background_updates, shows_background_location, allows_map_correction)) = back_tup.parse(input)?;
...
pub(crate) fn get_daemon_status_tracker(input: &[u8]) -> nom::IResult<&[u8], DaemonTrackerData> {
  ...
  let mut tup = (
    le_f64, le_u8, le_u8, le_u8, le_u8, le_u32, le_u32, le_u32, le_u32, le_i32, le_u8, le_u8, le_u8, le_u8,
  );
  let (
    location_data,
    (
