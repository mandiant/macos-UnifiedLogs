use nom::{
  bytes::complete::take,
  number::complete::{be_u8, be_u16, be_u32, be_u64, le_i8, le_i16, le_i32, le_i64, le_u8, le_u16},
};
...
pub fn parse_items_data<'a>(data: &'a [u8], flags: FirehoseFlags) -> nom::IResult<&'a [u8], RawFirehoseItemData<'a>> {
  ...
  // --- Backtrace skip ---
  let has_backtrace = flags.contains(FirehoseFlags::HAS_CONTEXT_DATA) || (input.len() > 3 && input[..3] == [1, 0, 18]);
...
pub fn parse_trace_items(data: &[u8]) -> Vec<RawFirehoseItem<'static>> {
  ...
  parse_trace_items_inner(&reversed).map(|(_, items)| items).unwrap_or_default()
}
