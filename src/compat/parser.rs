...
use crate::rewrite::logarchive::{load_file_buffers_by_uuid, load_timesync_data, load_uuidtext_buffers};
...
use super::unified_log::{
  CatalogInfo, CountVec, EventType, HeaderInfo, LogData, OversizeEntry, ParserError, TimesyncBoot,
...
pub fn build_log(
  ...
  for ov in &unified_data.oversize {
    oversize_cache
      .entries
      .insert((ov.data_ref_index, ov.first_proc_id, ov.second_proc_id), ov.data.clone());
  }
  ...
  if let Err(e) = visit_tracev3(
    ...
    |entry| {
      ...
      let timestamp = entry.timestamp().format("%Y-%m-%dT%H:%M:%S%.9fZ").to_string();
